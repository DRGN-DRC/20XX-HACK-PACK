...
v2.x of this code relocates the Stage Swap Table from the DOL to a file in the disc filesystem, and fixes random byte overwrites (when using 0xFF byte replacement mode).
[Achilles, DRGN]
...
/*
# Make sure file exists in disc filesystem
8033796c 000002f4 8033796c 0 DVDConvertPathToEntrynum

# Get size of file swap file
800163d8 000000cc 800163d8 0 File_GetSize

8037f1e4 00000060 8037f1e4 0 HSD_MemAlloc

8001668c 000000d4 8001668c 0 File_WaitForFileToLoad
3 args are filename ptr, file alloc ptr, and int ptr (returns file size)
*/
...
# Make sure file exists in disc filesystem
lis r15, <<SwapTableFileString>>@h		# Load pointer to file name string
ori r15, r15, <<SwapTableFileString>>@l
mr r3, r15
branchl r14, 0x8033796c					# DVDConvertPathToEntrynum; TOC entryNum now in r3
cmpwi r3,-1
beq END

# Get size of the stage swap file, and allocate space for it
mr r3, r15
branchl r14, 0x800163d8					# File_GetSize; size now in r3
mr r12, r3
branchl r14, 0x8037f1e4					# HSD_MemAlloc; file allocation pointer now in r3
mr r11, r3

# Load the stage swap file, and store a pointer to it
mr r3, r15
mr r4, r11
mr r5, r12
branchl r14, 0x8001668C					# File_WaitForFileToLoad; address of file is now in r3
stw r12, 0x14(r15)
